...
All metrics of Color Score (calculated from different subset strategies) are highly correlated with both Chlorophyll concentration and Symbiont density. 

For Chlorophyll concentration, Color Score calculated from the Full set has the highest correlation (.78), followed by that calculated from each Timepoint or Season individually (.75), and the lowest correlation (.71) was Color Score calculated for each Analysis set.  

For Symbiont density, Color Score calculated from either each Timepoint individually or each Analysis set had the highest correlations (0.71), followed by that calculated from the Full set (.69), then from each Season (.65).

To avoid issues with different sample sizes across Seasons or Analysis sets, comparing models with either Color Score calculated from the Full dataset or for each Timepoint individually. 
...
Chl_Full.mod.plot<-plot(ggpredict(Chl_Full.glm.gam.l , terms="Score_Full"), rawdata=TRUE, colors="darkgreen", jitter=TRUE)+
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        annotate("text", x=17, y=-.4, label=expression(bolditalic(paste("p < 0.001  ", eta^2, " = 0.37 "))),size=sig.sz, hjust = 0);Chl_Full.mod.plot
...
Chl_TP.glm.inv<-glm(Chl_ug.cm2~Score_TP, data=FullData, family=inverse.gaussian(link = "1/mu^2"))
Chl_TP.glm.inv.l<-glm(Chl_ug.cm2~Score_TP, data=FullData, family=inverse.gaussian(link = "log"))

#AIC Model Comparison
AIC(Chl_TP.glm.gaus, Chl_TP.glm.gaus.l, Chl_TP.glm.gaus.i, Chl_TP.glm.gam.l, Chl_TP.glm.gam.i,  Chl_TP.glm.inv, Chl_TP.glm.inv.l)
```
...
Color Score calculated for the Full dataset significantly predicts Chlorophyll (p< 2e-16) and explains 43% (Eta squared 0.43) of the variance. 
...
Chl_TP.mod.plot<-plot(ggpredict(Chl_TP.glm.gam.l , terms="Score_TP"), rawdata=TRUE, colors="lightgreen", jitter=TRUE)+
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.x=element_text(size=axis.txt.sz, colour="black"),
        axis.text.y=element_text(size=axis.txt.sz, colour="black"))+
        annotate("text", x=19, y=-.4, label=expression(bolditalic(paste("p < 0.001  ", eta^2, " = 0.43 "))),size=sig.sz, hjust = 0);Chl_TP.mod.plot
...
Color Score calculated for the Full dataset significantly predicts Chlorophyll (p< 2e-16) and explains 34% (Eta squared 0.34) of the variance. 
...
Sym_Full.mod.plot<-plot(ggpredict(Sym_Full.glm.gam.l , terms="Score_Full"), rawdata=TRUE, colors="darkorange3", jitter=TRUE)+
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.x=element_text(size=axis.txt.sz, colour="black"),
        axis.text.y=element_text(size=axis.txt.sz, colour="black"))+
        annotate("text", x=17, y=-.2, label=expression(bolditalic(paste("p < 0.001  ", eta^2, " = 0.34 "))),size=sig.sz, hjust = 0);Sym_Full.mod.plot
...
Sym_TP.mod.plot<-plot(ggpredict(Sym_TP.glm.gam.l , terms="Score_TP"), rawdata=TRUE, colors="darkorange", jitter=TRUE)+
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.x=element_text(size=axis.txt.sz, colour="black"),
        axis.text.y=element_text(size=axis.txt.sz, colour="black"))+
        annotate("text", x=19, y=-.2, label=expression(bolditalic(paste("p < 0.001  ", eta^2, " = 0.43 "))),size=sig.sz, hjust = 0);Sym_TP.mod.plot
...
Bleach_Color_Models_fig<-plot_grid(Chl_Full.mod.plot, Chl_TP.mod.plot,
                     ...
                     labels=c("A", "B", "C", "D", label_size=panel.lab.sz))

##Save Figure
ggsave(filename="Figures/Bleaching_Color_Models.png", plot=Bleach_Color_Models_fig, dpi=300, width=10, height=8, units="in")
