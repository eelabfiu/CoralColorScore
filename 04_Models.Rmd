...
### Predict Summer 2
```{r}
##Predicted values on Response scale (exp())
M1_Chl_predict<-data.frame(Predicted=exp(predict(Chl_W2.glm, newdata=data.frame(Score_TP=c(FullData$Score_TP[which(FullData$TimeP=="M1")])), type="response")))
...
cor.test(M1_Chl_predict$Predicted, M1_Chl_predict$Measured)
...
Chl_M1.pred.plot<-ggplot(M1_Chl_predict, aes(x=Predicted, y=Measured))+
  ...
  labs(x=expression(paste('Predicted Chlorophyll (\u03BCg cm'^-2*") ")), y=expression(paste('Measured Chlorophyll (\u03BCg cm'^-2*") ")), title="Summer")+
  theme_classic()+
  ylim(0,2)+
  xlim(1,2)+
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        annotate("text", x=1, y=1.5, label="r = 0.756  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0);Chl_M1.pred.plot
...
```{r}
##Predicted values on Response scale (exp())
M4_Chl_predict<-data.frame(Predicted=exp(predict(Chl_W2.glm, newdata=data.frame(Score_TP=c(FullData$Score_TP[which(FullData$TimeP=="M4")])), type="response")))

##Compare with Measured values 
M4_Chl_predict$Measured<-FullData$Chl_ug.cm2[which(FullData$TimeP=="M4")]

##Test correlation between Predicted and Measured values
cor.test(M4_Chl_predict$Predicted, M4_Chl_predict$Measured)
...
Chl_M4.pred.plot<-ggplot(M4_Chl_predict, aes(x=Predicted, y=Measured))+
  ...
  labs(x=expression(paste('Predicted Chlorophyll (\u03BCg cm'^-2*") ")), y=expression(paste('Measured Chlorophyll (\u03BCg cm'^-2*") ")), title="Winter")+
  theme_classic()+
  ylim(0,2)+
  xlim(1,2)+
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.y=element_text(size=axis.txt.sz, colour="black"),
        plot.title = element_text(size = plot.title.sz, colour="black", hjust = 0.5))+
        annotate("text", x=1, y=1.5, label="r = 0.696  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0);Chl_M4.pred.plot
...
```{r}
##Predicted values on Response scale (exp())
M1_Sym_predict<-data.frame(Predicted=exp(predict(Sym_W2.glm, newdata=data.frame(Score_TP=c(FullData$Score_TP[which(FullData$TimeP=="M1")])), type="response")))

##Compare with Measured values 
M1_Sym_predict$Measured<-FullData$Sym10.6_cm2[which(FullData$TimeP=="M1")]

##Test correlation between Predicted and Measured values
cor.test(M1_Sym_predict$Predicted, M1_Sym_predict$Measured)
...
Sym_M1.pred.plot<-ggplot(M1_Sym_predict, aes(x=Predicted, y=Measured))+
  ...
  labs(x=expression(paste('Predicted Symbionts (*10'^-6* ' cm'^-2*") ")), y=expression(paste('Measured Symbionts (*10'^-6* ' cm'^-2*") ")), title="Summer")+
  theme_classic()+
  xlim(1,2.15)+
  ylim(0, 1.15)+
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.y=element_text(size=axis.txt.sz, colour="black"),
        plot.title = element_text(size = plot.title.sz, colour="black", hjust = 0.5))+
        annotate("text", x=1, y=.8, label="r = 0.763  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0);Sym_M1.pred.plot
...
```{r}
##Predicted values on Response scale (exp())
M4_Sym_predict<-data.frame(Predicted=exp(predict(Sym_W2.glm, newdata=data.frame(Score_TP=c(FullData$Score_TP[which(FullData$TimeP=="M4")])), type="response")))

##Compare with Measured values 
M4_Sym_predict$Measured<-FullData$Sym10.6_cm2[which(FullData$TimeP=="M4")]

##Test correlation between Predicted and Measured values
cor.test(M4_Sym_predict$Predicted, M4_Sym_predict$Measured)
...
Sym_M4.pred.plot<-ggplot(M4_Sym_predict, aes(x=Predicted, y=Measured))+
  ...
  labs(x=expression(paste('Predicted Symbionts (*10'^-6* ' cm'^-2*") ")), y=expression(paste('Measured Symbionts (*10'^-6* ' cm'^-2*") ")), title="Winter")+
  theme_classic()+
  xlim(1.2,1.9)+
  ylim(0, .7)+
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.y=element_text(size=axis.txt.sz, colour="black"),
        plot.title = element_text(size = plot.title.sz, colour="black", hjust = 0.5))+
        annotate("text", x=1.2, y=.6, label="r = 0.610  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0);Sym_M4.pred.plot
...
#### Update for Panel
```{r}
Chl_M1.pred.plot.fig<-ggplot(M1_Chl_predict, aes(x=Predicted, y=Measured))+
    geom_point(size=point.sz, colour="lightgreen", pch=15)+ 
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.y=element_text(size=axis.txt.sz, colour="black"),
        plot.title = element_text(size = plot.title.sz, colour="black", hjust = 0.5))+
        annotate("text", x=1, y=1.5, label="r = 0.756  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0)

Chl_M4.pred.plot.fig<-ggplot(M4_Chl_predict, aes(x=Predicted, y=Measured))+
    geom_point(size=point.sz, colour="lightgreen", pch=17)+ 
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.y=element_text(size=axis.txt.sz, colour="black"),
        plot.title = element_text(size = plot.title.sz, colour="black", hjust = 0.5))+
        annotate("text", x=1, y=1.5, label="r = 0.696  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0)

Sym_M1.pred.plot.fig<-ggplot(M1_Sym_predict, aes(x=Predicted, y=Measured))+
    geom_point(size=point.sz, colour="darkorange", pch=15)+ 
  labs(x=expression(paste('Predicted Symbionts (cells *10'^-6* ' cm'^-2*") ")), y="Measured Symbionts", title=NULL)+
  theme_classic()+
  xlim(1,2.15)+
  ylim(0, 1.15)+
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.x=element_text(size=axis.txt.sz, colour="black"),
        axis.text.y=element_text(size=axis.txt.sz, colour="black"))+
        annotate("text", x=1, y=1, label="r = 0.763  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0)
...
Sym_M4.pred.plot.fig<-ggplot(M4_Sym_predict, aes(x=Predicted, y=Measured))+
  ...
  theme(axis.title.x = element_text(size = axis.title.sz), 
        ...
        axis.text.x=element_text(size=axis.txt.sz, colour="black"),
        axis.text.y=element_text(size=axis.txt.sz, colour="black"))+
        annotate("text", x=1.2, y=.6, label="r = 0.610  p < 0.001",  fontface="bold.italic", size=sig.sz, hjust = 0)
...
```{r}
##Create Panel
Bleach_Predict_fig<-plot_grid(Chl_M1.pred.plot.fig, Chl_M4.pred.plot.fig,
